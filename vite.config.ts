...
const siteConfiguration: FigmaSiteConfiguration = (() => {
  const p = './.figma/make/site.json'
  try { return existsSync(p) ? (JSON.parse(readFileSync(p, 'utf-8')) as FigmaSiteConfiguration) : {} } catch { return {} }
})()
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "Naga ERP"
